-- Create Vehicle Contracts Tables
-- Run this script in PostgreSQL to create the tables

-- First drop if exists (optional, remove if you want to keep existing data)
DROP TABLE IF EXISTS vehicle_contract_routes CASCADE;
...
CREATE TABLE vehicle_contract_routes (
    id SERIAL PRIMARY KEY,
...
CREATE INDEX idx_vehicle_contracts_company ON vehicle_contracts(vehicle_company_id);
CREATE INDEX idx_vehicle_contracts_dates ON vehicle_contracts(start_date, end_date);
CREATE INDEX idx_vehicle_contracts_code ON vehicle_contracts(contract_code);

CREATE INDEX idx_vehicle_contract_routes_contract ON vehicle_contract_routes(vehicle_contract_id);
CREATE INDEX idx_vehicle_contract_routes_from ON vehicle_contract_routes(from_location);
CREATE INDEX idx_vehicle_contract_routes_to ON vehicle_contract_routes(to_location);
