...
function createCost($conn, $data) {
    ...
    try {
        ...
        $checkQuery = "SELECT id FROM costs WHERE LOWER(name) = LOWER($1)";
        $checkResult = pg_query_params($conn, $checkQuery, [$name]);
        if ($checkResult && pg_num_rows($checkResult) > 0) {
            echo json_encode(['success' => false, 'message' => getApiTranslation('cost_name_exists')]);
            pg_query($conn, "ROLLBACK");
            return;
        }
...
// Save cost periods
function saveCostPeriods($conn, $cost_id, $periods) {
    foreach ($periods as $period) {
...
function updateCost($conn, $data) {
    ...
    try {
        ...
        // Check if cost name already exists for another cost
        $checkQuery = "SELECT id FROM costs WHERE LOWER(name) = LOWER($1) AND id != $2";
        ...
        // Save periods if provided
        if (isset($data['periods']) && is_array($data['periods'])) {
            saveCostPeriods($conn, $id, $data['periods']);
        }
